...
import useFoods from "../hooks/usefetchfoods";

const FoodAdmin = () => {
  const API_URL = import.meta.env.VITE_API_URL;

  const { allfoods, loading, error, refetch, setfoodanddrinks } = useFoods();
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API_URL}/food/deletefood/${id}`);
      toast.success("Food item deleted successfully");

      // Update state by filtering out the deleted item
      setfoodanddrinks((prevFoods) =>
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold bg-white bg-clip-text text-transparent">
                  Food & Drinks Admin
                </h1>
                <p className="text-sm sm:text-base text-slate-400">
                  Manage your cinema's culinary offerings
                </p>
              </div>
            </div>
          </div>
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    <div
                      className={`w-11 h-6 rounded-full transition-colors duration-200 ${
                        formData.available ? "bg-emerald-500" : "bg-slate-600"
                      }`}
                    >
                      <div
                ...
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  ...
                  <div className="flex-shrink-0 mx-auto sm:mx-0">
                    <div className="w-20 h-20 sm:w-24 sm:h-24 bg-slate-700/50 rounded-2xl overflow-hidden border border-slate-600/50">
                      <img
                        src={food.foodImage || "https://via.placeholder.com/96"}
                        alt={food.name || food}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                      <div>
                        ...
                        <div className="flex items-center justify-center sm:justify-start gap-2 mt-1">
                          <span
                            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-gradient-to-r ${getCategoryColor(
                              food.category || "snack"
                            )}`}
                          >
